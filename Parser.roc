...
import rtils.StrUtils exposing [lowercase]
...
blank = open_bracket |> rhs(P.integer) |> lhs(dot) |> lhs(P.whitespace) |> both(P.one_or_more(char_excluding_bracket) |> P.map(|bs| bs |> Str.from_utf8)) |> lhs(close_bracket) |> P.map(|(i, s)| Ok(Blank({ index: i, part_of_speech: lowercase(s), answer: "" })))
